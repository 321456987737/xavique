...
export default function CheckoutPage() {
  ...
  const validateForm = () => {
    ...
    if (!billingSame) {
      const billingRequired = ['billingLastName', 'billingAddress', 'billingCity'];
      const billingMissing = billingRequired.filter(field => !formData[field]);
      
      if (billingMissing.length > 0) {
        return { valid: false, error: `Missing billing fields: ${billingMissing.join(', ')}` };
      }
    }
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-gray-400 text-sm">
                            ...
                            {item.selectedOptions?.color || "N/A"} &bull; Size:{" "}
                            {item.selectedOptions?.size || "N/A"}
                          </p>
                        </div>
                      </div>
